...
**要考** 内外人事组黄杰
分析类通过以下几种方式表现出来：
1. **外部实体**（例如，其他系统、设备、人员），它们产生或消耗计算机系统使用的信息。
2. **内部事物**（例如，报告、显示器、信函、信号），它们是问题的信息领域的一部分。
3. **发生事件**（例如，财产转移或一系列机器人移动的完成），它们发生在系统操作的上下文中。
4. **角色**（例如，经理、工程师、销售员），由与系统交互的人扮演。
5. **组织单元**，在应用程序中**相关的组织单位**（例如，部门、组、团队）。（例如，项目管理系统的一个小组）
6. **环境**，确定问题的**背景环境**和系统的**整体功能的位置**（例如，制造车间或装车码头）。（例如，SafeHome系统中的传感器位置）
7. 定义了一类**对象或相关类的结构**（例如，传感器、四轮车或计算机）。（例如，控制面板）

![](./ref/classHomwSafeDemo.PNG)
看动词是不是实义动词，看是不是归属于哪个类
下划线是名词

由哪**6个特性**帮我找出来是不是分析类**要考** 留服多公公基
...
- **低耦合多个属性**。在需求分析过程中，重点应放在“重要”信息上；具有单个属性的类实际上在设计过程中可能有用，但在分析活动中最好将其表示为另一个类的属性。
